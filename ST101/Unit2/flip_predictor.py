...
class FlipPredictor(object):
    ...
    #the probabilities of flipping each coin
        ...
        for i in range(coins_count):
            if result == 'H':
                joints.append(self.coins[i]*self.probs[i])
            else:
                joints.append((1 - self.coins[i])*self.probs[i])
        normaliser = sum(joints)
        for i in range(coins_count):
            self.probs[i] = joints[i]/normaliser
