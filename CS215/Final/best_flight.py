...
def find_best_flights(flights, origin, destination):
    ...
    shortest_paths = find_shortest_path(G, origin)
    return shortest_paths[destination][2]

def create_graph(flights):
    G = {}
    for number, orig, dest, start, end, cost in flights:
        fmt = '%H:%M'
        d_start = datetime.datetime(*strptime(start, fmt)[:6])
        d_end = datetime.datetime(*strptime(end, fmt)[:6])
        d_diff = (d_end - d_start).seconds/60
        params = [number, cost, d_diff]
        if orig not in G:
            G[orig] = {}
        (G[orig])[dest] = params
#        if dest not in G:
            ...
            for x in G[w]:
                if x not in final_dist:
                    ...
                    if x not in dist_so_far:
                        dist_so_far[x] = []
                        dist_so_far[x].append(final_dist[w][0] + path_cost)
                        dist_so_far[x].append(final_dist[w][1] + path_time)
                        dist_so_far[x].append(final_dist[w][2] + [num_race])
                        heappush(heap,(final_dist[w][0] + path_cost, final_dist[w][1] + path_time, x))
                    elif final_dist[w][0] + path_cost < dist_so_far[x][0] or (final_dist[w][0] + path_cost == dist_so_far[x][0] and final_dist[w][1] + path_time < dist_so_far[x][1]):
                        dist_so_far[x].append(final_dist[w][0] + path_cost)
                        dist_so_far[x].append(final_dist[w][1] + path_time)
                        dist_so_far[x].append(final_dist[w][2] + [num_race])
                        heappush(heap,(final_dist[w][0] + path_cost, final_dist[w][1] + path_time, x))
